...
{% macro polars__alter_column_type(relation,column_name,new_column_type) -%}
...
{% macro polars__check_schema_exists(information_schema,schema) -%}
...
{% macro polars__create_schema(relation) -%}
...
{% macro polars__drop_relation(relation) -%}
...
{% macro polars__drop_schema(relation) -%}
...
{% macro polars__get_columns_in_relation(relation) -%}
...
{% macro polars__list_relations_without_caching(schema_relation) -%}
'''creates a table of relations withough using local caching.'''
{% endmacro %}

{% macro polars__list_schemas(database) -%}
...
{% macro polars__rename_relation(from_relation, to_relation) -%}
...
{% macro polars__truncate_relation(relation) -%}
