...
class BaseSimpleSnapshotBase:
    ...
    def seeds(self):
        """
        This seed file contains all records needed for tests,
        including records which will be inserted after the
        initial snapshot. This table will only need to be loaded once
        at the class level. It will never be altered, hence requires
        no further setup or teardown.
        """
    ...
    def models(self):
        """
        This will be the working base table. It will be altered by each test,
        hence will require setup and teardown at the test case level.
        See `self._setup_method(self, project)`.
        """
    ...
    def _setup_method(self, project):
        ...
        Only load the first 20 `seed` records into `fact`;
        withhold 10 records as "new" (e.g. to test inserts).

        Make the project a class variable to simplify function calls
        and make the code more readable.For some reason, this doesn't work in
        the class-scoped fixture, but does in the function-scoped fixture.
        """
    ...
    def get_snapshot_records(
        self, select: str = None, where: str = None
    ) -> list[tuple]:
    ...
    def _assert_results(
        ...
        """
        All test cases are checked by considering whether a source
        record's id has a value in `dbt_valid_to` in `snapshot`.
        Each id can fall into one of the following cases:

        - The id has only one record in `snapshot`; it has a value in
        `dbt_valid_to`
        ...
        - The id has only one record in `snapshot`;
          it does not have a value in `dbt_valid_to`
            ...
            - the record was updated in the source, but not in a way
              that is tracked (e.g. via `strategy='check'`)
        - The id has two records in `snapshot`; one has a value in
          `dbt_valid_to`, the other does not
            - the record was altered in the source in a way that is tracked
            - the record was hard deleted and revived

        Note: Because of the third scenario, ids may show up in both arguments
        of this method.

        Args:
            ids_with_current_snapshot_records: a list/set/etc.
            of ids which aren't end-dated
            ids_with_closed_out_snapshot_records: a list/set/etc.
            of ids which are end-dated
        """
        records = set(
            self.get_snapshot_records("id, dbt_valid_to is null as is_current")
        )
...
class BaseSimpleSnapshot(BaseSimpleSnapshotBase):
    ...
    def test_updates_are_captured_by_snapshot(self, project):
        """
        Update the last 5 records. Show that all ids are current,
        but the last 5 reflect updates.
        """
    ...
    def test_inserts_are_captured_by_snapshot(self, project):
        """
        Insert 10 records. Show that there are 30 records in `snapshot`,
    ...
    def test_deletes_are_captured_by_snapshot(self, project):
        """
        Hard delete the last five records. Show that there are now only 15
        current records and 5 expired records.
        """
    ...
    def test_revives_are_captured_by_snapshot(self, project):
        """
        Delete the last five records and run snapshot to collect
        that information, then revive 3 of those records.
    ...
    def test_new_column_captured_by_snapshot(self, project):
        ...
        Show that all ids are current, but the last 10 reflect updates
        and the first 10 don't i.e. if the column is added, but not updated, the
        record doesn't reflect that it's updated
        """
...
class BaseSnapshotCheck(BaseSimpleSnapshotBase):
    ...
    def test_column_selection_is_reflected_in_snapshot(self, project):
        ...
        (records 6-10 updated on both)
        Show that all ids are current, and only the tracked column updates are
        ...
        self.update_fact_records(
            {"email": "left(email, 3)"}, "id between 6 and 15"
        )  # tracked
        run_dbt(["snapshot"])
        self._assert_results(
            ids_with_current_snapshot_records=range(1, 21),
