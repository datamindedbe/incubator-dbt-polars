...
# Same scenario as models__merge_exclude_columns_sql, but expressed as an
# allow-list (merge_update_columns) instead of an exclude-list. Excluding
# 'msg' is equivalent to only updating 'id' and 'color', so the expected
# result is identical.
...
class BaseMergeExcludeColumns:
    ...
    def get_test_fields(self, project, seed, incremental_model, update_sql_file):
        ...
        seed_rows = polars_relation_row_count(project.adapter, seed)
...
class TestMergeExcludeColumns(BaseMergeExcludeColumns, PolarsTestMixin):
    pass


class TestMergeUpdateColumns(BaseMergeExcludeColumns, PolarsTestMixin):
    """Same scenario as TestMergeExcludeColumns, expressed as an allow-list."""

    @pytest.fixture(scope="class")
    def models(self):
        return {"merge_exclude_columns.sql": models__merge_update_columns_sql}


class TestMergeColumnsMutuallyExclusive(PolarsTestMixin):
    @pytest.fixture(scope="class")
    def models(self):
        return {
    ...
    def test__merge_update_and_exclude_columns_conflict(self, project):
        run_dbt(["run"])  # first run: not incremental yet, builds fine

        results = run_dbt(["run"], expect_pass=False)
        assert results[0].status == RunStatus.Error
        assert "merge_update_columns" in results[0].message
        assert "merge_exclude_columns" in results[0].message
