{% materialization test, adapter='polars' %}
  ...
  {% if should_store_failures() %}
    ...
    {% if store_failures_as != 'table' %}
      {{ exceptions.raise_compiler_error(
        "store_failures_as='" ~ store_failures_as ~ "' is not supported for singular tests on the "
  ...
  {% else %}
    {% set res, table = adapter.polars_execute_sql_test(sql, model['extra_ctes'], limit, fail_calc, warn_if, error_if, target_relation) %}
  {% endif %}
