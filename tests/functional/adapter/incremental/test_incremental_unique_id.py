...
# Rows appended directly via the catalog to simulate new source data arriving,
# in place of running the raw SQL INSERT statements dbt-core's version uses
# (Polars' SQLContext only supports SELECT-style queries, not DML).
seed_updates = {
    "duplicate_insert": [
        {
            "state": "CT",
...
{% if is_incremental() %}
with cutoff as (
    select max(last_visit_date) as cutoff_date from {{ this }}
)
{% endif %}
...
{% if is_incremental() %}
    cross join cutoff
    where last_visit_date > cutoff.cutoff_date
{% endif %}
...
select seed.* from {{ ref('seed') }} as seed
...
class BaseIncrementalUniqueKey:
    ...
    def get_test_fields(
        ...
        polars_append_rows(project.adapter, seed, seed_updates[update_sql_file])
    ...
    def test__unary_unique_key_list(self, project):
        ...
        test_case_fields = self.get_test_fields(
            ...
            update_sql_file="duplicate_insert",
            opt_model_count=self.update_incremental_model(
                "unique_key_list__inplace_overwrite"
            ),
        )
        self.check_scenario_correctness(expected_fields, test_case_fields, project)
...
class TestIncrementalUniqueKey(BaseIncrementalUniqueKey, PolarsTestMixin):
    pass
