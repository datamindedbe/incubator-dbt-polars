...
class BasePersistDocsBase:
    ...
    def models(self):
        return {
            ...
            "incremental_model.sql": _MODELS__INCREMENTAL,
        }
    ...
    def _assert_has_incremental_comments(self, incremental_node):
        incremental_comment = incremental_node["metadata"]["comment"]
        assert incremental_comment.startswith("Incremental model description")

        incremental_id_comment = incremental_node["columns"]["id"]["comment"]
        assert incremental_id_comment.startswith("id Column description")

        self._assert_common_comments(incremental_comment, incremental_id_comment)

    def _assert_has_no_comments(self, node):
        assert node["metadata"]["comment"] is None
...
class BasePersistDocs(BasePersistDocsBase):
    ...
    def test_has_comments(self, project):
        ...
        incremental_node = catalog_data["nodes"]["model.test.incremental_model"]
        self._assert_has_incremental_comments(incremental_node)

        no_docs_node = catalog_data["nodes"]["model.test.no_docs_model"]
        self._assert_has_no_comments(no_docs_node)
