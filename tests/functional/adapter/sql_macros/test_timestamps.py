...
select {{ current_timestamp() }} as current_timestamp,
       {{ current_timestamp_in_utc_backcompat() }}
           as current_timestamp_in_utc_backcompat,
...
class BaseCurrentTimestamps:
    ...
    def test_current_timestamps(self, project, models, expected_schema, expected_sql):
        ...
        if expected_sql:
            ...
            assert expected_sql_check == generated_sql_check, (
...
@pytest.mark.xfail(
    ...
    reason=(
        "dbt-polars intentionally does not support current_timestamp() (and its "
        "backcompat variants) via SQL — Polars' SQL engine has no runtime clock "
        "function. Revisit if Polars adds support."
    ),
)
